...
class PaymentRepository extends BaseRepository implements PaymentInterface{
    ...
    public function payment_summary($client = 'all', $from_date = '', $to_date = ''){
        $query = "SELECT payments.*, invoices.client_id, payment_methods.name AS method_name, clients.name AS client_name, invoices.invoice_no,invoices.currency FROM payments
