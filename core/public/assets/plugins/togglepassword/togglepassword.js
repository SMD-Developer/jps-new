...
(function ($) {
    $.fn.togglepassword = function (btnclass) {
        this.each(function(){
            ...
            let value = $(this).val();
            let content = '<i class="fa fa-eye" style="font-size: x-large"></i>';
            let title = 'Show password';
            btnclass = btnclass ?? '';
            return $(this).replaceWith(
                $('<div class="d-flex"></div>')
                    .append($('<input type="password" placeholder="Kata Laluan" class="' + classlist + '" id="' + id + '" name="' + name + '" value="' + value + '"' + autofocus + '>'))
...
$(() => {
    $(document).on('click', '[data-role="togglepassword"]', function () {
        ...
        $(this).html('<i class="fa fa-eye' + (is_password ? '-slash' : '') + '" style="font-size: x-large"></i>');
        $(this).attr('title', is_password ? 'Hide password' : 'Show password' );
    });
});
